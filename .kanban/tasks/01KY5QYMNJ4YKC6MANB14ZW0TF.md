...
The no-leak backstop for detached commands, split out as a precursor so it lands and tests independently against the blocking `ShellRunner.run` — before anything detaches.
...
## Tests
- [x] `Tests/ShellToolTests/ProcessRegistryTests.swift` — sweep kills a live private-registry child; register/deregister lifecycle across a run; ESRCH tolerance
